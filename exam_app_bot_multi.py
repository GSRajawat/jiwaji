...
try:
    ...
    st.error("❌ Could not import NorenApiPy. Make sure api_helper.py is in the correct location.")
    NorenApiPy = None
...
if not all([SUPABASE_URL, SUPABASE_KEY, ENCRYPTION_KEY]):
    st.error("❌ Missing required secrets. Please configure .streamlit/secrets.toml")
    st.stop()
...
def save_flattrade_credentials(user_id: str, flattrade_user_id: str, session_token: str, password: str) -> dict:
    """Save or update Flattrade credentials for a user."""
    try:
        # Encrypt sensitive data
        encrypted_session = encrypt_data(session_token)
        encrypted_password = encrypt_data(password)
        
        # Check if credentials already exist
        existing = supabase.table('user_flattrade_credentials').select('*').eq('user_id', user_id).execute()
        
        if existing.data:
            # Update existing credentials
            result = supabase.table('user_flattrade_credentials').update({
                'flattrade_user_id': flattrade_user_id,
                'flattrade_session_token': encrypted_session,
                'flattrade_password': encrypted_password,
                'updated_at': datetime.now(timezone.utc).isoformat()
            }).eq('user_id', user_id).execute()
        else:
            # Insert new credentials
            result = supabase.table('user_flattrade_credentials').insert({
                'user_id': user_id,
                'flattrade_user_id': flattrade_user_id,
                'flattrade_session_token': encrypted_session,
                'flattrade_password': encrypted_password
        ...
        logging.error(f"Error saving Flattrade credentials: {e}")
        return {'success': False, 'error': str(e)}

def get_flattrade_credentials(user_id: str) -> dict:
    """Retrieve and decrypt Flattrade credentials for a user."""
    try:
        result = supabase.table('user_flattrade_credentials').select('*').eq('user_id', user_id).eq('is_active', True).execute()
        
        if result.data:
            creds = result.data[0]
            return {
                'success': True,
                'flattrade_user_id': creds['flattrade_user_id'],
                'session_token': decrypt_data(creds['flattrade_session_token']),
                'password': decrypt_data(creds['flattrade_password'])
            }
        else:
            return {'success': False, 'error': 'No credentials found'}
    except Exception as e:
        logging.error(f"Error retrieving Flattrade credentials: {e}")
        return {'success': False, 'error': str(e)}

def init_user_flattrade_api(user_id: str) -> tuple:
    """Initialize Flattrade API for a specific user."""
    if NorenApiPy is None:
        return None, "NorenApiPy not available"
    
    try:
        creds = get_flattrade_credentials(user_id)
        
        if not creds['success']:
            return None, f"Failed to get credentials: {creds.get('error')}"
        
        # Create a new API instance
        api = NorenApiPy()
        
        # Set session with user-specific credentials
        ret = api.set_session(
            userid=creds['flattrade_user_id'],
            password=creds['password'],
            usertoken=creds['session_token']
        )
        ...
        if ret:
            ...
            return None, "Failed to set Flattrade session. Please check your credentials."
            
    except Exception as e:
        logging.error(f"Error initializing Flattrade API: {e}")
        return None, str(e)
...
def init_session_state():
    ...
    if 'flattrade_api' not in st.session_state:
        st.session_state.flattrade_api = None
    if 'api_error' not in st.session_state:
        st.session_state.api_error = None
    if 'strategy' not in st.session_state:
        st.session_state.strategy = None
    if 'order_conditions' not in st.session_state:
        st.session_state.order_conditions = {}
...
def ensure_flattrade_api():
    """Ensure Flattrade API is initialized for the current user."""
    if st.session_state.flattrade_api is None and st.session_state.api_error is None:
        with st.spinner("Initializing Flattrade API..."):
            api, error = init_user_flattrade_api(st.session_state.user['id'])
            
            if api:
                st.session_state.flattrade_api = api
                st.session_state.api_error = None
                return True, None
            else:
        ...
        return False, st.session_state.api_error
    else:
        return True, None
...
# --- Helper Functions (Abbreviated - Include all your helper functions here) ---
...
def get_fno_stocks_list():
    ...
    try:
        ...
        logging.error(f"Error during stock list generation: {e}")
        return ['RELIANCE', 'TCS', 'HDFCBANK', 'INFY', 'ICICIBANK']
            ...
            if new_username and new_password:
                if new_password == confirm_password:
                    with st.spinner("Creating account..."):
                        result = create_user(new_username, new_password, new_email or None)
...
def flattrade_credentials_setup():
    """Setup Flattrade credentials for the user."""
    st.subheader("📊 Flattrade API Credentials")
    
    # Check if credentials already exist
    creds = get_flattrade_credentials(st.session_state.user['id'])
    
    if creds['success']:
        st.success("✅ Flattrade credentials are configured")
        col1, col2 = st.columns(2)
        with col1:
            st.info(f"**Flattrade User ID:** {creds['flattrade_user_id']}")
        with col2:
            if st.button("Update Credentials"):
                st.session_state.show_cred_form = True
    else:
        st.warning("⚠️ Please configure your Flattrade credentials to start trading")
        st.session_state.show_cred_form = True
    
    if st.session_state.get('show_cred_form', not creds['success']):
        with st.form("flattrade_creds_form"):
            st.write("Enter your Flattrade API credentials:")
            
            ft_user_id = st.text_input(
                "Flattrade User ID", 
                value=creds.get('flattrade_user_id', '') if creds['success'] else '',
                help="Your Flattrade user ID (e.g., FZ12345)"
            )
            ft_session = st.text_input(
                "Session Token", 
                type="password",
                help="Your Flattrade session token"
            )
            ft_password = st.text_input(
                "Flattrade Password", 
                type="password",
                help="Your Flattrade password"
            )
            ...
            if submitted:
                if ft_user_id and ft_session and ft_password:
                    result = save_flattrade_credentials(
                    ...
                    if result['success']:
                        ...
                        st.session_state.show_cred_form = False
                        # Clear the API cache so it reinitializes with new creds
                        st.session_state.flattrade_api = None
                        st.session_state.api_error = None
                        st.session_state.strategy = None
                        time_module.sleep(1)
...
def trading_dashboard():
    ...
    # 1. API CHECK (Already included)
    success, error = ensure_flattrade_api()
    if not success:
        st.error(f"❌ {error}")
        return
    
    api = st.session_state.flattrade_api
    
    # 2. STRATEGY INITIALIZATION (Already included)
    if st.session_state.strategy is None:
        st.session_state.strategy = FnOBreakoutStrategy()
    
    strategy = st.session_state.strategy
    
    # 3. ADD YOUR SIDEBAR CONTROLS HERE
    with st.sidebar:
        ...
        if enable_daily_volume_screen:
            volume_multiplier = st.slider("Volume Multiplier (X times 3-day Avg)", 0.5, 10.0, 2.0, 0.5, key="vol_mult_slider", help="Current cumulative volume must be this many times the 3-day average volume for the same time.")
             ...
             volume_multiplier = 2.0 # Default value, ignored if screen is off
        ...
        auto_scan = st.toggle("🤖 Auto Scan (on load)", True, key="auto_scan_toggle", help="Automatically scans and generates the watchlist.")
        auto_trading = st.toggle("Enable Auto Trading (Multi-Stock)", True, key="auto_trade_toggle")
        ...
        auto_refresh = st.toggle("🔄 Auto Refresh (5s)", value=True, key="auto_refresh_toggle")
...
def main_app():
    ...
    st.sidebar.write(f"**User ID:** {st.session_state.user['id'][:8]}...")
...
def main():
    st.set_page_config(
        page_title="Multi-User Trading App",
        page_icon="📊",
        layout="wide",
